...
import {
	refetchChainConfigQuery,
	refetchKeplrConfigQuery,
	IChainConfigQuery,
	IKeplrConfigQuery,
} from "@andromedaprotocol/gql/dist/__generated/react";
import { GasPrice } from "@cosmjs/stargate/build/fee";
import type { AccountData, Keplr } from "@keplr-wallet/types";
import { create } from "zustand";
import { chain_id } from "./static";
...
export const connectAndromedaClient = async () => {
	try {
		...
		try {
			...
			const keplrConfig = await apolloClient.query<IKeplrConfigQuery>(
				refetchKeplrConfigQuery({
					identifier: chainId,
				})
			);
			await keplr.experimentalSuggestChain(
				keplrConfig.data.keplrConfigs.config
			);
		}

		const config = (
			await apolloClient.query<IChainConfigQuery>(
				refetchChainConfigQuery({
					identifier: chainId,
				})
			)
		...
		const client =
			state.client ||
			new (await import("@andromedaprotocol/andromeda.js")).default();
